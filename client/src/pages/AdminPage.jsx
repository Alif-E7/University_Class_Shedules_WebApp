...
import { auth, analytics, importApi, departments, teachers, courses, offerings, rooms, terms, users, schedules as schedulesApi } from '../api';
...
/* ── Generic CRUD ── */
function CrudPage({ title, icon, api: apiModule, fields, idField = 'id', extraFilters }) {
...
function ImportWizard() {
  ...
  const [importType, setImportType] = useState('all'); // 'all', 'teachers', 'courses', 'offerings', 'schedules'
  ...
  useEffect(() => { terms.list().then(setTermList).catch(() => {}); }, []);
  ...
  const handleConfirm = async () => {
    ...
    setErr('');
    try {
      await importApi.confirm(preview.batch_id, selectedTerm || null);
        ...
        <div className="card">
          ...
          <div className="field" style={{ marginTop: '1rem' }}>
            ...
            <select value={selectedTerm} onChange={e => setSelectedTerm(e.target.value)}>
              ...
              {termList.map(t => <option key={t.term_id} value={t.term_id}>{t.academic_year} — {t.term_name}{t.is_active ? ' (active)' : ''}</option>)}
            </select>
          </div>
          ...
          <p className="muted" style={{ fontSize: '0.8rem', marginTop: '1rem', padding: '0.5rem', background: 'var(--bg-secondary)', borderRadius: '6px' }}>
            ...
            {importType === 'all' && 'Requires a multi-sheet Excel file. Sheets: Department, Teachers, Courses, Offerings, Schedule.'}
            {importType === 'teachers' && 'Requires an Excel file (or first sheet) with columns: staff_no, full_name, designation, email, phone, office_room.'}
            {importType === 'courses' && 'Requires an Excel file (or first sheet) with columns: course_code, course_title, credit, semester.'}
            {importType === 'offerings' && 'Requires an Excel file (or first sheet) with columns: course_code, staff_no, term, section.'}
            {importType === 'schedules' && 'Requires an Excel file (or first sheet) with columns: course_code, section, day, start_time, end_time, room_number, building.'}
          </p>

          <div className="upload-zone" style={{ marginTop: '1rem' }}>
            <div className="upload-zone-icon">📄</div>
            <input type="file" accept=".xlsx,.xls" onChange={e => setFile(e.target.files?.[0] || null)} style={{ border: 'none', padding: 0 }} />
          </div>
      ...
      {step === 1 && preview && (
        <div className="card">
          ...
          <div className="grid grid-4" style={{ margin: '1rem 0' }}>
            {[
              { v: preview.summary.teachers, l: 'Teachers' },
              { v: preview.summary.courses, l: 'Courses' },
              { v: preview.summary.offerings, l: 'Offerings' },
              { v: preview.summary.schedules, l: 'Schedules' },
            ].map(s => (
...
export default function AdminPage() {
  ...
  useEffect(() => {
    if (loggedIn) {
      ...
      terms.list().then(setTermList).catch(() => {});
    }
  }, [loggedIn]);
  ...
  const termOptions = termList.map(t => ({ value: t.term_id, label: `${t.academic_year} ${t.term_name}${t.is_active ? ' ✓' : ''}` }));
        ...
        <Routes>
          ...
          {isCentral && <>
            <Route path="departments" element={
              <CrudPage title="Departments" icon="🏛️" api={departments} idField="department_id" fields={[
            ...
            <Route path="terms" element={
              <CrudPage title="Terms" icon="📅" api={terms} idField="term_id" fields={[
                { key: 'academic_year', label: 'Academic Year', required: true, placeholder: '2025-2026' },
                { key: 'term_name', label: 'Term Name', required: true, placeholder: 'Spring' },
                { key: 'is_active', label: 'Active', type: 'select', options: [{ value: 'true', label: 'Yes' }, { value: 'false', label: 'No' }], render: v => v ? <span className="badge badge-success">Active</span> : <span className="badge badge-muted">Inactive</span> },
              ]} />
            } />
          </>}

          <Route path="teachers" element={
            <CrudPage title="Teachers" icon="👩‍🏫" api={teachers} idField="teacher_id" fields={[
          ...
          <Route path="courses" element={
            <CrudPage title="Courses" icon="📖" api={courses} idField="course_id" fields={[
              ...
              { key: 'semester', label: 'Semester', placeholder: '1-1' },
              ...(isCentral ? [{ key: 'department_id', label: 'Department', type: 'select', options: deptOptions, render: (v, item) => item.department_code || '—' }] : []),
            ]} />
          } />
          <Route path="offerings" element={
            <CrudPage title="Course Offerings" icon="📋" api={offerings} idField="offering_id" fields={[
              ...
              { key: 'term_name', label: 'Term', render: (v, item) => `${item.academic_year || ''} ${item.term_name || ''}` },
              ...
              { key: 'term_id', label: 'Term', hideInTable: true, type: 'select', options: termOptions },
            ]} />
          } />
          <Route path="schedules" element={
            <CrudPage title="Schedule Slots" icon="🕐" api={schedulesApi} idField="slot_id" fields={[
          ...
          <Route path="rooms" element={
            <CrudPage title="Rooms" icon="🚪" api={rooms} idField="room_id" fields={[
              { key: 'building', label: 'Building', placeholder: 'Main' },
